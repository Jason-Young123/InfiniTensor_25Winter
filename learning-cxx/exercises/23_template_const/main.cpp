...
struct Tensor {
    ...
    Tensor(unsigned int const shape_[N]) {
		...
		//或者用accumulate,需要<numeric>头文件
		//size = std::accumulate(std::begin(shape_), std::end(shape_), 1, std::multiplies<int>());
...
private:
    unsigned int data_index(unsigned int const indices[N]) const {
		...
		/*for(int i = N - 1; i >= 0; i--){//涉及到减到0,i必须用unsigned
            ASSERT(indices[i] < shape[i], "Invalid index");
			index += coef * indices[i];
			coef *= shape[i];
		}*/
        return index;
    }
